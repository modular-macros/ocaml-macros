...
let complete_type_list ?(allow_absent=false) env nl1 lv2 mty2 nl2 tl2 =
  ...
  let env' = Env.add_module Asttypes.Nonstatic (* macros: not sure *) id2 mty2 env in
    ...
    ty'.desc <-
      ...
      | Tvariant row ->
          ...
          with Not_found ->
            ...
            let is_static = static_row row in
            let more' = if is_static then newgenty Tnil else more in
            (* Return a new copy *)
            let row =
