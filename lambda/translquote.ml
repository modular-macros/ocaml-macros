...
let merge_fvs (l: Lambda.lambda list) (base: Lambda.lambda) =
  List.fold_left (fun acc x -> merge acc (snd x)) base l
...
let rec quote_lambda lam =
  ...
  match lam with
  | Lvar id -> let fv = free_var id in
               seq (check fv)
                   (pair (mkblock ~tag [Lvar id]) (free_var id))
  | Lmutvar id -> let fv = free_var id in
                  seq (check fv)
                      (pair (mkblock ~tag [Lvar id]) fv)
  | Lconst _ as cst -> pair (const cst) (empty ())
  | Lapply lapp -> quote_lambda_apply lapp %>>
                     fun c fvs -> seq (check fvs)
                                      (pair (mkblock ~tag [c]) fvs)
  | Lfunction lfunction -> quote_lfunction lfunction %>>
                             fun c fvs -> seq (check fvs)
                                              (pair (mkblock ~tag [c]) fvs)
  | Llet (lk, vk, id, e1, e2) ->
      term e1 %>> fun c1 fvs1 ->
        gensym id @@ (
          new_scope [id] (term e2) %>> fun c2 fvs2 ->
            merge fvs1 fvs2 >>> fun fvs ->
              seq (check fvs)
                  (let args = [const lk; const vk; Lvar id; c1; c2] in
                   pair (mkblock ~tag args) fvs))
  | Lmutlet (vk, id, e1, e2) ->
      term e1 %>> fun c1 fvs1 ->
        gensym id @@ (
          new_scope [id] (term e2) %>> fun c2 fvs2 ->
  ...
  | Lletrec (binds, e) ->
      ...
      let mklam = fun {def;_} -> new_scope params (quote_lfunction def) in
      let ts = List.map mklam binds in
      ...
      List.fold_right gensym params @@
        new_scope params (term e) %>> fun c fvs1 ->
          merge_fvs ts fvs1 >>> fun fvs ->
            seq (check fvs)
                (let mkpair = fun (id, def) -> pair (Lvar id) def in
                 let list_defs = list mkpair defs in
                 pair (mkblock ~tag [list_defs; c]) fvs)
  | Lprim (p, es, loc) ->
      let ts = List.map term es in
      pair (list fst ts) (merge_fvs ts (empty ())) %>>
        fun cs fvs ->
          seq (check fvs)
              (pair (mkblock ~tag [const p; cs; const loc]) fvs)
  | Lswitch (e, sw, loc) ->
      term e %>> fun c fvs1 ->
        quote_lambda_switch sw %>> fun sw fvs2 ->
        merge fvs1 fvs2 >>> fun fvs ->
          seq (check fvs)
              (pair (mkblock ~tag [c; sw; const loc]) fvs)
  | Lstringswitch (e, cases, eopt, loc) ->
      let strs, es = List.split cases in
      let ts = List.map term es in
      let mkpair = fun (str, t) -> pair (const str) (fst t) in
      let cs = list mkpair (List.combine strs ts) in
      term e %>> fun c fvs1 ->
  ...
  | Lstaticraise (i, es) ->
      ...
      fun cs fvs ->
        seq (check fvs)
            (pair (mkblock ~tag [const i; cs]) fvs)
  ...
  | Ltrywith (e1, id, e2) ->
      term e1 %>> fun c1 fvs1 ->
        gensym id @@
          new_scope [id] (term e2) %>> fun c2 fvs2 ->
            merge fvs1 fvs2 >>> fun fvs ->
              seq (check fvs) (pair (mkblock ~tag [c1; Lvar id; c2]) fvs)
  ...
  | Lwhile (e1, e2) ->
      term e1 %>> fun c1 fvs1 ->
        term e2 %>> fun c2 fvs2 ->
          merge fvs1 fvs2 >>> fun fvs ->
            seq (check fvs) (pair (mkblock ~tag [c1; c2]) fvs)
  | Lfor (id, e1, e2, dir, e3) ->
      term e1 %>> fun c1 fvs1 ->
        term e2 %>> fun c2 fvs2 ->
          gensym id @@
            (new_scope [id] (term e3) %>> fun c3 fvs3 ->
               merge (merge fvs1 fvs2) fvs3 >>> fun fvs ->
                 let args = [Lvar id; c1; c2; const dir; c3] in
                 seq (check fvs) (pair (mkblock ~tag args) fvs))
  | Lassign (id, e) ->
      term e %>> fun c fvs ->
        seq (check fvs) (pair (mkblock ~tag [const id; c]) fvs)
        ...
        term e2 %>> fun c2 fvs2 ->
          merge fvs1 fvs2 >>> fun fvs3 ->
            pair (list fst ts) (merge_fvs ts fvs3) %>> fun cs fvs ->
              let args = [const mk; c1; c2; cs; const loc] in
              seq (check fvs) (pair (mkblock ~tag args) fvs)
  | Levent (e, _)
...
and quote_lfunction { kind; params; return; body; attr; loc } =
  ...
  List.fold_right gensym vars @@
    new_scope vars (term body) %>> fun c fvs ->
...
and quote_lambda_apply { ap_func; ap_args; ap_loc; ap_tailcall;
  ...
  term ap_func %>> fun c1 fvs1 ->
    ...
    pair (list fst ts) (merge_fvs ts fvs1) %>> fun cs fvs ->
      let args = [c1; cs;
                  const ap_loc;
                  const ap_tailcall;
                  const ap_inlined;
...
and quote_lambda_switch { sw_numconsts; sw_consts; sw_numblocks;
  ...
  let mkpair = fun (i, t) -> pair (const i) (fst t) in
  pair (list mkpair consts) (merge_fvs ts1 (empty ())) %>> fun cs_consts fvs1 ->
    pair (list mkpair blocks) (merge_fvs ts2 fvs1) %>> fun cs_blocks fvs2 ->
...
let fv (env : Env.t) (t : Typedtree.expression) : Ident.Set.t =
  ...
  let rec id = function
    ...
    | Env.Adot (x,_) -> id x
  in
  let expr iter : Typedtree.expression -> unit = function
...
let transl_close_expression _loc params body =
  (Ident.Set.fold gensym_no_rename params) @@
    fst (new_scope (Ident.Set.to_list params)
                   (body %>> fun c fvs -> seq (check fvs) (pair c fvs)))
