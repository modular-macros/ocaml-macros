...
let compile_world_dummies = ref Ident.Set.empty

let record_compile_world_dummy id =
  compile_world_dummies := Ident.Set.add id !compile_world_dummies

let is_compile_world_dummy = function
  | Lvar id ->
      in_compile_world () && Ident.Set.mem id !compile_world_dummies
  | Lconst c -> in_compile_world () && c = const_unit
  | _ -> false

let module_address_is_dummy env path =
  let rec root = function
    | Env.Aident id -> Ident.Set.mem id !compile_world_dummies
    | Env.Adot (a, _) -> root a
  in
...
let transl_module_path loc env path =
  ...
  | None ->
      ...
      | Some lam -> lam
      | None ->
          if compile_world && module_address_is_dummy env path then
            Lconst const_unit
          else Lambda.transl_module_path loc env path
...
let rec apply_coercion loc strict restr arg =
  match restr with
    Tcoerce_none ->
      arg
  | (Tcoerce_structure _ | Tcoerce_functor _)
    when is_compile_world_dummy arg ->
      arg
...
let dummy_slots ids next fields =
  List.iter record_compile_world_dummy ids;
        ...
        (fun fields ->
           if in_compile_world () then
             let rec rebind pos fields = function
               | [] -> next fields
               | id :: rest ->
                   if not (Ident.Set.mem id macro_ids) then
                     record_compile_world_dummy id;
...
let reset () =
  ...
  compile_world_dummies := Ident.Set.empty;
  Env.reset_required_globals ();
  Translprim.clear_used_primitives ();
  reset_template_applications ();
