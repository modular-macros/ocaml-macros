  ...
  let static_translate_expr ~scopes exp =
    ...
    let expr_iter iter (e : Typedtree.expression ) =
      ...
      | Texp_splice { spl_exp; spl_index = Some idx } ->
          (* MACO-NOTE for each toplevel splice, we generate a runtime
            scope_extrussion_check([translated lambda]). The translated
            lambda comes from the normal translcore, which will do the
            special translation of quotes. *)
          let body =
...
type splice_funs = {
  transl_splices : scopes:scopes -> structure_item -> (int * lambda) list;
  insert_splices : (int * lambda) list -> lambda -> lambda;
}

let splices_funs () =
  if !transl_stage = 0 then
    { transl_splices = (fun ~scopes:_ _ -> []);
...
and transl_structure ~scopes loc fields cc rootpath final_env = function
    [] ->
        ...
        match cc with
          Tcoerce_none ->
            Lprim(Pmakeblock(0, Immutable, None),
              ...
              List.length fields
          (* MACO-REV: lsp hints that type is staging_level but it is just int.
             review how to generally deal with this for clarity*)
        | Tcoerce_structure(pos_cc_list, id_pos_list) ->
      ...
      size
  | item :: rem ->
      let maybe_splice_funs = splices_funs () in
      let str_lam, size =
     ...
     | Tstr_modtype _
      ...
      | Tstr_attribute _ ->
          transl_structure ~scopes loc fields cc rootpath final_env rem
      in
      (maybe_splice_funs.insert_splices (maybe_splice_funs.transl_splices ~scopes item) str_lam, size)
...
let transl_implementation sf module_name (str, cc) =
  (* MACO-TODO move static into different entry point *)
