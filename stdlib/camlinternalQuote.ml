...
module Lambda = struct
  ...
  module Constant = struct
    ...
    let unmarshal s : t = Marshal.from_string s 0
  ...
  module Lambda = struct
    ...
    let application ap_loc ap_func ap_args ap_should_be_tailcall
    ...
    let function_ loc kind params body attr =
      Lfunction {
        kind;
        params;
        body;
    ...
    let switch lam ~numconsts ~consts ~numblocks ~blocks ~failaction =
      Lswitch (lam,
        {
    ...
    let stringswitch loc lam cases lam_opt =
      Lstringswitch (lam, cases, lam_opt, loc)

    let staticraise i lams =
      Lstaticraise (i, lams)

    let staticcatch lam l body =
      Lstaticcatch (lam, l, body)

    let trywith lam exn_name body =
      Ltrywith (lam, exn_name, body)

    let ifthenelse cond ift iff =
      Lifthenelse (cond, ift, iff)

    let sequence lam lam' =
      Lsequence (lam, lam')

    let while_ cond body =
      Lwhile (cond, body)

    let for_ name init final direction body =
      Lfor (name, init, final, direction, body)
